...
const LossRecoveryCalculator: React.FC = () => {
    ...
    const [lossInputValue, setLossInputValue] = useState<number>(10);
    ...
    const handlelossInputValueInputChange = (raw: string) => {
        ...
        const val = Number(clean.replace(/,/g, ''));
        if (!isNaN(val) && val >= 0)
            setLossInputValue(Math.min(val, 1_000_000_000));
    };
    ...
    const handleReturnRateChange = (raw: string) => {
        ...
        if (!isNaN(val)) setExpectedReturnRate(val);
    };
    ...
    const burdenTooltip = (
        <div className="max-w-[24rem] space-y-2 text-sm">
            <p className="font-bold">
                {locale === 'vi'
                    ? 'Tại sao có “gánh nặng gỡ lỗ”?'
                    : 'Why is there a "recovery burden"?'}
            </p>
            <p>
                {locale === 'vi'
            ...
            <p className="italic">
                % Gánh Nặng Gỡ Lỗ = % Lãi Cần Hòa Vốn − % Lỗ / Danh Mục Ban Đầu
            </p>
        </div>
    );

    const expectedReturnTooltip = (
        <div className="max-w-[24rem] space-y-1 text-sm">
            <p>
                {locale === 'vi'
                    ? 'Lợi nhuận kỳ vọng mỗi kỳ (e.g., mỗi tháng, quý). Không tự động cộng dồn nếu chọn Lãi Đơn.'
                    : 'Expected return per period (e.g., monthly, quarterly). Not reinvested in Simple Interest mode.'}
            </p>
        </div>
    );
            ...
            <div className="w-full md:w-1/4">
                <Card className="bg-transparent dark:bg-transparent h-full">
                    <CardHeader>
                        ...
                        <CardDescription>
                            {locale === 'vi'
                                ? 'Nhập thông tin để phân tích gỡ lỗ'
                                : 'Enter details to analyze loss recovery'}
                        </CardDescription>
                    </CardHeader>
                    ...
                    <CardContent className="space-y-6">
                            ...
                            <AppTooltips
                                ...
                                trigger={
                                    <Button
                                        variant="ghost"
                                        size="icon"
                                        className="absolute right-2 top-1/2 -translate-y-1/2 dark:hover:bg-transparent"
                                    >
                        ...
                        <div className="relative">
                            ...
                            <AppDropdown
                                ...
                                labelVisible={false}
                                buttonClassName="w-10 h-8 absolute right-2 top-1/2 -translate-y-1/2 bg-muted/30 hover:bg-muted text-xs"
                        ...
                        <div className="relative">
                            ...
                            <AppDropdown
                                ...
                                labelVisible={false}
                                buttonClassName="w-12 h-8 absolute right-10 top-1/2 -translate-y-1/2 bg-muted/30 hover:bg-muted text-xs"
                    ...
                    <CardFooter className="border-t">
                        ...
                        <div className="grid grid-cols-1 gap-3 w-full">
                            ...
                            <SummaryCard
                                title={
                                    locale === 'vi'
                                        ? 'Số Kỳ Cần Gỡ Lỗ (Lãi Kép)'
                                        : 'Periods to Break Even (Compound)'
                                }
                                value={
                                    isFinite(periodsToBreakEven())
                                        ? `${periodsToBreakEven().toFixed(2)} ${returnRateUnit === 'year' ? (locale === 'vi' ? 'năm' : 'yr') : returnRateUnit === 'month' ? (locale === 'vi' ? 'tháng' : 'mo') : locale === 'vi' ? 'ngày' : 'day'}`
                                        : '∞'
                                }
                                color="text-blue-500"
                            />
            ...
            <div className="w-full md:w-3/4 flex flex-col gap-4">
                ...
                <Card className="bg-transparent dark:bg-transparent flex-1">
                    <CardHeader>
                        ...
                        <CardDescription>
                            {locale === 'vi'
                                ? 'So sánh Lãi Đơn và Lãi Kép trong việc gỡ lỗ'
                                : 'Compare Simple vs. Compound Interest for recovery'}
                        </CardDescription>
                    </CardHeader>
                    <CardContent style={{ height: height }} className="pl-8">
                        <ResponsiveContainer
                            width="100%"
                            height="100%"
                            className={'relative'}
                        >
                            ...
                            >
                                ...
                                <XAxis
                                    dataKey="period"
                                    label={{
                                        value: `${locale === 'vi' ? `Kỳ (${renderReturnRateUnit()})` : `Period (${renderReturnRateUnit()})`}`,
                                ...
                                />
                                <p className="absolute left-0 top-1/2 transform -translate-x-1/2 -translate-y-1/2 -rotate-90 font-bold leading-relaxed text-brand-text dark:text-[#666]">
                                    {locale === 'vi'
                                        ? 'Giá Trị Danh Mục'
                                        : 'Portfolio Value'}
                                </p>
...
}> = ({ title, value, color = 'text-brand-text', tooltip }) => {
    return (
        <div
            className={cn(
                'p-3 rounded-md border flex gap-2 items-center',
                color
            )}
        >
